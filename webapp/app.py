...
def infer_category_options(df: pd.DataFrame, col: str) -> List[str]:
    if col not in df.columns:
        return ["None"]

    vals = [model_to_display_value(v) for v in df[col].drop_duplicates().tolist()]
    vals = sorted(set(v for v in vals if v))

    if "None" not in vals:
        vals = ["None"] + vals
    else:
        vals = ["None"] + [v for v in vals if v != "None"]
...
def categorical_input(df: pd.DataFrame, col: str):
    opts = infer_category_options(df, col)
...
def build_selected_input_panel(df: pd.DataFrame, bundles: Dict[str, dict]) -> Tuple[bool, Dict[str, object]]:
    ...
    with tab_basic:
        row1 = st.columns(2)
        with row1[0]:
            values["Monomer A1 type"] = categorical_input(df, "Monomer A1 type")
        with row1[1]:
            values["Monomer A2 type"] = categorical_input(df, "Monomer A2 type")
        ...
        row3 = st.columns(2)
        with row3[0]:
            values["Monomer B type"] = categorical_input(df, "Monomer B type")
        with row3[1]:
        ...
        values["Organic solvent type"] = categorical_input(df, "Organic solvent type")

    with tab_additive:
        cols = st.columns(2)
        with cols[0]:
            values["Additive X1 type in aqueous phase"] = categorical_input(df, "Additive X1 type in aqueous phase")
            ...
            values["Additive X2 type in aqueous phase"] = categorical_input(df, "Additive X2 type in aqueous phase")
        ...
        with cols[1]:
            values["Additive Y type in organic phase"] = categorical_input(df, "Additive Y type in organic phase")
            ...
            values["Nanomaterials type in aqueous phase"] = categorical_input(df, "Nanomaterials type in aqueous phase")
            ...
            values["Nanomaterials type in organic phase"] = categorical_input(df, "Nanomaterials type in organic phase")
